...
let () =
    describe "int reducer" (fun _ ->
        ...
        test "jsonify" (fun _ ->
            expect (jsonify reducer_int 42) |> toBe (Js.Json.parse "42")));
    describe "string reducer" (fun _ ->
        ...
        test "jsonify" (fun _ ->
            expect (jsonify reducer_str "foo") |> toBe (Js.Json.parse "\"foo\"")));
    describe "record.initial" (fun () ->
        ...
        test "jsonify" (fun () ->
            expect (jsonify reducer_record {x=0; y=0}) |> toEqual (Js.Json.parse "{ \"x\": 0, \"y\": 0 }")))
